...
function hideChildrenOf(element) {
  ...
  const deps = element && element.nextElementSibling && element.nextElementSibling.children;
  ...
  const btn = element && element.firstElementChild;

  if (btn) {
    btn.textContent = BRANCH_CLOSED_SYMBOL;
  }
}
...
function showChildrenOf(element) {
  ...
  if (btn) {
    btn.textContent = BRANCH_OPEN_SYMBOL;
  }
}
...
/**
 * Copies the selected dependency path to clipboard
 */
function copyPathToClipboard() {
  navigator.clipboard.writeText(dependencyPath.join(' > '));
}
